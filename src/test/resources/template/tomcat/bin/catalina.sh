...
# -Xms  4096m   初始化堆大小 推荐为可用内存的1/4的80%~100%，16G内存，配置为4G
# -Xmx  4096m   最大堆大小 -Xms –Xmx 一般的要将-Xms和-Xmx选项设置为相同堆内存分配 (访问量比较大时设为一致)
# -Xss  256k    表示每个 Java 线程堆栈大小，JDK 5.0 以后每个线程堆栈大小为 1M，以前每个线程堆栈大小为 256K。根据应用的线程所需内存大小进行调整，在相同物理内存下，减小这个值能生成更多的线程，但是操作系统对一个进程内的线程数还是有限制的，不能无限生成，经验值在 3000~5000 左右。一般小的应用， 如果栈不是很深， 应该是128k 够用的，大的应用建议使用 256k 或 512K，一般不易设置超过 1M，要不然容易出现out ofmemory。这个选项对性能影响比较大，需要严格的测试。
# -Xmn  256m    -Xmn：新生代的内存空间大小，注意：此处的大小是（eden+ 2 survivor space)。与 jmap -heap 中显示的 New gen 是不同的。整个堆大小 = 新生代大小 + 老生代大小 + 永久代大小。在保证堆大小不变的情况下，增大新生代后，将会减小老生代大小。此值对系统性能影响较大，Sun官方推荐配置为整个堆的 3/8。

# -XX:PermSize  256m    持久代非堆的初始值[物理内存的1/64]
# -XX:MaxPermSize 512m 持久代 非堆的最大值[物理内存的1/4]

# -XX:MetaSpaceSize 256m    java8开始不再有 持久代  由元空间代替
...
# -XX:+UseBiasedLocking     启用一个优化了的线程
# -XX:MaxTenuringThreshold  31  设置垃圾最大年龄

# 垃圾收集模式    ！JDK5.0以后，JVM会根据当前系统配置进行判断。 linux 64位 默认使用 并发处理
...
# -XX:+UseParallelGC -XX:+UseParallelOldGC -XX:ParallelGCThreads=设置并行垃圾回收的线程数。此值可以设置与机器处理器数量相等。

# 并发处理器：适用情况：“对响应时间有高要求”，多CPU、对应用响应时间有较高要求的中、大型应用。举例：Web服务器/应用服务器、电信交换、集成开发环境。
# -XX:+UseConcMarkSweepGC   使用并行收集器 在执行GC时 应用不停止    通过设置-XX:CMSInitiatingOccupancyFraction=指定还有多少剩余堆时开始执行并发收集
...
JAVA_OPTS="-server -Xms2048m -Xmx4096m -Xss256k -XX:PermSize=256m -XX:MaxPermSize=1024m -XX:NewSize=512m -XX:MaxNewSize=2048m -XX:+DisableExplicitGC -XX:+AggressiveOpts -XX:+UseBiasedLocking -XX:MaxTenuringThreshold=31 -XX:+UseConcMarkSweepGC -XX:+UseParNewGC -XX:+CMSParallelRemarkEnabled -XX:+UseCMSCompactAtFullCollection -XX:LargePageSizeInBytes=128m -XX:+UseFastAccessorMethods -XX:+UseCMSInitiatingOccupancyOnly -Djava.awt.headless=true"
